...
# Totals
p1 = df1.pivot_table(index=["sent_by"], values=["url"], aggfunc=lambda x: len(x.unique())).reset_index()
...
wt = daily_posts_wt.add(weekly_posts_wt).add(total_posts_wt).sort_values(ascending=False)
p3.insert(loc=0, column="Score", value=wt)
p3 = p3.sort_values(by=["Score"], ascending=[False]).reset_index(drop=True).reset_index(names="Rank")
...
try:    
    worksheet2 = sh.add_worksheet(title="Ranking", rows=p3.shape[0], cols=6,)
